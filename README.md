...
### Describes
ExcFile是个基于Netty实现的简单文件传输系统，目前此系统包含有以下功能：
* 大文件上传
* 断点续传

### Requests
* 服务器(运行excfile-server)默认开启10888端口，可在配置文件中修改
* 客户端(运行excfile-client)

### Getting started
1. 设置服务器自定义目录 /excfile-server/src/main/resources/中的配置文件
    ```yaml
   # 网盘初始化的根路径，不存在则创建
   net-disk:
     root: 自定义目录
    ```
1. 运行/excfile-server/src/main/java/com/kanouakira/server/ExcfileServerApplication.java
